...
public class Array01P146 {
	...
	public static void main(String[] args) {
		...
		// 배열.length를 이용해 배열 크기를 알 수 있습니다.
		
		// 배열의 선언1(길이 확정 X)
		int b = 5; // 기본형 변수(값을 스택에 직접 들고있다.)
		System.out.println(b);
		int a[] = {1, 2, 3, 4}; // a변수에 1, 2, 3, 4를 동시에 넣기. 참조형 변수(값을 힙에 저장)
		// Arrays.toString()이 없으면 콘솔창에 주소만 나옵니다.
		System.out.println(Arrays.toString(a));
		System.out.println(a); // 스택에 저장된 주소값만 나온다.
		
		// 배열의 선언2(길이 확정 X)
		double[] c = {5.1, 6.3, 7.2, 8.77};
		// c변수에 저장된 주소가 아닌 실제 자료를 조회해보세요.
		System.out.println(Arrays.toString(c));
		System.out.println(c); // 스택에 저장된 주소값만 나온다.
		
		// 배열의 생성(길이 확정) // 자동으로 0이 채워집니다.
		int[] d = new int[10]; // 10칸짜리 배열 생성
		// 일단 10칸이 자동으로 전부 0이 채워짐
