...
public class Warrior { // 정의부, 실행은 Main에서 할 예정임.
	...
	// 나머지는 hp 20, strangth 3, lv 1, money 0을
	// 자동으로 입력받도록 해주세요.
	...
	public void hunt() { // 메서드는 public으로 해야 실행이 가능하다.
		if (hp == 0) {
			System.out.println("hp가 0이므로 사냥이 불가능합니다.");
		} else {
			hp -= 2; // hunt에 정의된 규칙대로 정해진 hp만 변경되도록 되어있다.
			money += 10;
			System.out.println("사냥결과 hp가 " + hp + " 이고, 소지금은 " + money + "입니다.");
		}
	}
