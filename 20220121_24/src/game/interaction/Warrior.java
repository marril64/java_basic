...
public class Warrior {
	// Warrior의 특성으로
	// 체력(hp), 마나(mp), 공격력(atk), 경험치(exp), 레벨(lv)을 넣어주세요.
	private int hp;
	private int mp;
	private int atk;
	private int exp;
	...
	public void huntOrc(Orc orc) {
		...
		if (orc.getHp() <= 0) {
			...
			this.exp += 10; // 오크쪽에 경험치를 추가해줘서 getExp()로 처리하는게 더 낫다.
			System.out.println("전사의 남은 체력 : " + this.hp);
			System.out.println("전사의 경험치 : " + this.exp);
			//return; else문으로 처리하는 방법도 있다.
		} else {
		
		// 오크 공격력만큼 자신의 체력을 차감해줍니다.
		this.hp -= orc.getAtk(); // 200번지의 getAtk();
		System.out.println("전사의 남은 체력 : " + this.hp);
		}
	}
	...
	// 타 몬스터쪽에서 전사스탯을 조회할 수 있도록 Getter/Setter를 만들어주세요.
	public int getHp() {
		return hp;
	}
